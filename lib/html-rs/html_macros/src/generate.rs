...
impl Generator {
        ...
        match markup {
            ...
            Markup::Block(Block { markups, outer_span }) => {
                if markups.iter().any(|markup| matches!(*markup, Markup::Let { .. })) {
                    self.block(Block { markups, outer_span }, build);
                } else {
    ...
    fn block(&self, Block { markups, outer_span }: Block, build: &mut Builder) {
        let block = {
            let mut build = self.builder();
    ...
    fn splice(&self, expr: TokenStream, build: &mut Builder) {
        ...
        build.push_tokens(quote!(
            html::macro_private::render_to!(&(#expr), &mut #output_ident);
        ));
    }
