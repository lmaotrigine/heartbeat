...
fn attributes() {
    let alt = "Pinkie Pie";
    let result = html! { img src="pinkie.png" alt=(alt); };
    assert_eq!(result.into_string(), r#"<img src="pinkie.png" alt="Pinkie Pie">"#);
}
...
fn nested_macro_invocation() {
    let best_pony = "Pinkie Pie";
    let result = html! { (format!("{best_pony} is the best pony")) };
    assert_eq!(result.into_string(), "Pinkie Pie is the best pony");
}
