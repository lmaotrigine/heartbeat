...
fn simple_elements() {
    ...
    assert_eq!(result.into_string(), "<p><b>pickle</b>barrel<i>kumquat</i></p>");
}
...
fn toggle_empty_attributes() {
    ...
    assert_eq!(
        result.into_string(),
        concat!(r#"<input checked>"#, r#"<input>"#, r#"<input checked>"#, r#"<input>"#)
    );
}
...
fn toggle_empty_attributes_braces() {
    ...
    let result = html! { input checked[Html { rocks: true }.rocks]; };
...
fn optional_attribute_braces() {
    ...
    let result = html! { input value=[Kitty { cuteness: Some(9000) }.cuteness]; };
...
fn hyphens_in_attribute_names() {
    ...
    assert_eq!(result.into_string(), r#"<this sentence-is="false" of-course></this>"#);
}

#[test]
fn class_shorthand() {
    let result = html! { p { "Hi, " span.name { "Lyra" } "!" } };
    assert_eq!(result.into_string(), r#"<p>Hi, <span class="name">Lyra</span>!</p>"#);
}
...
fn hyphens_in_class_names() {
    ...
    assert_eq!(result.into_string(), r#"<p class="rocks-these are--my--rocks">yes</p>"#);
}

#[test]
fn class_string() {
    let result = html! { h1."pinkie-123" { "Pinkie Pie" } };
    assert_eq!(result.into_string(), r#"<h1 class="pinkie-123">Pinkie Pie</h1>"#);
}
...
fn toggle_classes_braces() {
    ...
    let result = html! { p.rocks[Html { rocks: true }.rocks] { "Awesome!" } };
...
fn id_shorthand() {
    ...
    assert_eq!(result.into_string(), r#"<p>Hi, <span id="thing">Lyra</span>!</p>"#);
}

#[test]
fn id_string() {
    let result = html! { h1 #"pinkie-123" { "Pinkie Pie" } };
    assert_eq!(result.into_string(), r#"<h1 id="pinkie-123">Pinkie Pie</h1>"#);
}
